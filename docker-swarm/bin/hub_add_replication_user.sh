...
TIMEOUT=${TIMEOUT:-10}
HUB_POSTGRES_VERSION=${HUB_POSTGRES_VERSION:-16-2.8}
...
sleep_count=0
until docker exec -i ${container_id} pg_isready -q -U postgres ; do
    sleep_count=$(( ${sleep_count} + 1 ))
    [ ${sleep_count} -gt ${TIMEOUT} ] && fail "Database server in container ${container_id} not ready after ${TIMEOUT} seconds." 7
    sleep 1
done

docker exec -i ${container_id} psql -U postgres -c "CREATE USER blackduck_replication REPLICATION CONNECTION LIMIT 5 PASSWORD '$new_password'"
